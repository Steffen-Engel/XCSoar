...
public:
  ...
  virtual void Prepare(ContainerWindow &parent, const PixelRect &rc) noexcept override;
};


void
ManageyAllWidget::Prepare([[maybe_unused]] ContainerWindow &parent, [[maybe_unused]] const PixelRect &rc) noexcept
{
  tIdent Ident;
  ...
  AddButton(_("Calibrate Mag"), [this]() {device.CalibrateMag();});
  AddButton(_("Calibrate Acc"), [this]() {device.CalibrateAcc();});
  AddButton(_("Calibrate IAS"), [this]() {device.CalibrateIAS();});
  AddButton(_("Reboot"),        [this]() {device.Restart();});
}

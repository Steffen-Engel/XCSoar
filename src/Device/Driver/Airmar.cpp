...
Copyright_License {
  ...
  as p ublished by the Free Software Foundation; either version 2
...
class NMEA_WXDevice final : public AbstractDevice {
  Port &port;
...
public:
  explicit NMEA_WXDevice(Port &_port):port(_port) {}
...
bool
NMEA_WXDevice::WIMDA(NMEAInputLine &line, NMEAInfo &info)
{
  /*
  ...
  if (ch == 'B')
  {
    // barometric pressure delivered, not used actually
  }
...
bool
NMEA_WXDevice::ParseNMEA(const char *_line, NMEAInfo &info)
{
...
static Device *
NMEA_WXCreateOnPort(const DeviceConfig &config, Port &com_port)
{
  return new NMEA_WXDevice(com_port);
}
